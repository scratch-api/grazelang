...
use crate::{
    ...
    parser::{
        ...
        cst::{
            ...
            DataDeclarationScope, DictionaryTypeError, EMPTY_ISTRING_REF, Expression,
...
impl GetPos for GrazeSb3GeneratorError {
    fn get_source_span(&self) -> &SourceSpan {
        match self {
            ...
            GrazeSb3GeneratorError::DictionaryTypeError { error } => error.get_source_span(),
            GrazeSb3GeneratorError::InvalidConstantExpression {
                expression: _,
                source,
...
pub struct GrazeSb3GeneratorContext {
    ...
    pub lint_context: GrazeSb3GeneratorLintContext,
...
impl GrazeSb3GeneratorContext {
    ...
    ) -> Result<Self, (GrazeSb3GeneratorCreationError, Vec<GrazeSourceMessage>)> {
    ...
    pub fn canonicalize_resource_directory(
        settings: &mut GrazeBuildSettings,
    ...
    ($context:expr, $entry:expr, $pattern:pat => $value:expr, $pattern_2:pat => $value_2:expr) => {
        match $entry {
            $pattern => match $value {
                ...
                Err(err) => {
                    emit_error!(
                        GrazeSb3GeneratorError::DictionaryTypeError { error: err },
                        $context
                    );
                    None
                }
...
pub mod helpers {
    ...
    pub fn check_lint_for_repeated_data_name(
        context: &mut GrazeSb3GeneratorContext,
        ...
        if let Some(value) = context.lint_context.data_names.get_mut(name) {
            ...
            let old_value = *value;
            if data_name_usage.variable {
                value.variable = true;
            }
            if data_name_usage.list {
                value.list = true;
            }
            if old_value.global != data_name_usage.global
...
impl GrazeVisitor<GrazeSb3GeneratorContext, GrazeSb3GeneratorError> for GrazeSb3Generator {
                ...
                SingleAssignment::Var(scope, var, expression) => {
                    wrap_in_statement(context, |context, parent, this_id| {
                        ...
                        let SimpleCallableKnownBlockSignature(opcode, param, known_params) =
                            known_block
                                .resolve_for_assignment(Default::default(), context)
                                .unwrap();
                        let mut fields = HashMap::new();
                        let mut inputs = HashMap::new();
                        add_params(context, known_params.iter(), &mut inputs, &mut fields)?;
        ...
        let skip_param = if params.len() != 1 {
            if params.is_empty() && value.1.is_empty() {
                ...
                return Err(GrazeSb3GeneratorError::IncorrectParamCount {
                    unexpected: 1,
                    expected: params.len(),
                    source_span: value.0.span_to(value.1),
                });
            }
        } else {
            false
    ...
    fn visit_custom_block_definition(
        ...
        wrap_in_scope(context, |context| {
            ...
            for (key, arg) in arguments {
                ...
                use_symbol_as(context, arg, key, false);
            }
            use_symbol_as(
                context,
                arguments_symbol_id,
    ...
    fn visit_isolated_expression(
        ...
        with_known_block!(context, context.pop_param().unwrap(), param_source_span, value => {
            match value.resolve_for_input(param_source_span, context) {
                grazelang_types::KnownBlockInput::PrimitiveInput(mut sb3_primitive_block) => {
                ...
                grazelang_types::KnownBlockInput::BlockRef(_) => (),
                grazelang_types::KnownBlockInput::SimpleBlock(opcode, params) => {
    ...
    fn visit_top_level_statement_stage(
        ...
        wrap_in_scope(context, |context| {
            ...
            for (key, symbol_id) in namespace {
                if context
                    .symbol_table
            ...
            for (key, symbol_id) in insert_symbols {
                use_symbol_as(context, symbol_id, key, false);
            }
    ...
    fn visit_config_statement(
        ...
        let mut target = context.current_sb3_target.take().unwrap();
        if is_stage {
            if let Some(backdrop) = extract_data_from_dictionary_value!(
                context,
                data.remove("backdrop"),
                Some((_, value)) => value.to_literal(),
                value => value
            ) {
            ...
            if let Some(layer_order) =
                extract_f64_entry_from_dictionary_data(context, &mut data, "layer_order")?
            {
                target.layer_order = layer_order as usize;
            }
            if let Some(text_to_speech_language) = extract_data_from_dictionary_value!(
                context,
                data.remove("text_to_speech_language"),
                Some((_, value)) => value.to_literal(),
                value => value.get_non_empty().map(|value| value.get_string_value().to_string())
            ) {
                target.text_to_speech_language = text_to_speech_language;
            }
            if let Some(video_transparency) = extract_optional_f64_entry_from_dictionary_data(
                context,
                &mut data,
                "video_transparency",
            )? {
                target.video_transparency = video_transparency;
            }
            if let Some(video_state) = extract_data_from_dictionary_value!(
                context,
                data.remove("video_state"),
                Some((_, value)) => value.to_literal(),
                value => value
            ) {
                ...
                let video_state = video_state
                    .get_non_empty()
                    .map(|value| value.get_string_value().to_string());
                ...
                target.video_state = video_state;
            }
        } else {
            if let Some(costume) = extract_data_from_dictionary_value!(
                context,
                data.remove("costume"),
                Some((_, value)) => value.to_literal(),
                value => value
            ...
            if let Some(y_position) =
                extract_optional_f64_entry_from_dictionary_data(context, &mut data, "y_position")?
            {
                target.y = y_position;
            }
            if let Some(direction) =
                extract_optional_f64_entry_from_dictionary_data(context, &mut data, "direction")?
            {
                target.direction = direction;
            }
            if let Some(size) =
                extract_optional_f64_entry_from_dictionary_data(context, &mut data, "size")?
            {
                target.size = size;
            }
            if let Some(volume) =
            ...
            if let Some(rotation_style) = extract_data_from_dictionary_value!(
                context,
                data.remove("rotation_style"),
                Some((_, value)) => value.to_literal(),
                value => value
            ) {
                ...
                let rotation_style = rotation_style
                    .get_non_empty()
                    .map(|value| value.get_string_value().to_string());
                if let Some(rotation_style) = &rotation_style
                    && !matches!(
                ...
                target.rotation_style = rotation_style;
            }
        }
        context.current_sb3_target.replace(target);
    ...
    fn visit_monitor_declaration(
        ...
        let mut data = get_data_from_dictionary(context, value.3, |key| {
            if let Some(new_key) = match key.as_str() {
                ...
                "discrete" => Some(literal!("is_discrete")),
                _ => None,
            } {
                new_key
            } else {
        ...
        if let Some(id) = extract_data_from_dictionary_value!(
            context,
            data.remove("id"),
            Some((_, value)) => value.to_literal(),
            value => value.get_string_value().to_string()
        ) {
            monitor.id = id;
        }
        if let Some(mode) = extract_data_from_dictionary_value!(
            context,
            data.remove("mode"),
            Some((_, value)) => value.to_literal(),
            value => value
        ...
        if let Some(opcode) = extract_data_from_dictionary_value!(
            context,
            data.remove("opcode"),
            Some((_, value)) => value.to_literal(),
            value => value.get_string_value().to_string()
        ...
        if let Some(sprite_name) = extract_data_from_dictionary_value!(
            context,
            data.remove("sprite_name"),
            Some((_, value)) => value.to_literal(),
            value => value.get_non_empty().map(|value| value.get_string_value().to_string())
        ) {
            monitor.sprite_name = sprite_name;
        }
        if let Some(value) = extract_data_from_dictionary_value!(
            context,
            data.remove("value"),
            Some((_, value)) => value.to_literal_or_list(),
            value => value
        ) {
            match value {
                cst::DictionaryValueLiteralOrList::Literal(value) => {
                    if matches!(monitor.value, Sb3MonitorValue::List(..)) {
                        emit_message_eager(
                            ...
                            GrazeSourceMessage::Warning(
                                GrazeSourceWarning::Specific(
                                    SpecificGrazeWarning::PrimitiveValueForListMonitor,
                                    *value.get_source_span(),
                                ),
                                None,
                            ),
                ...
                cst::DictionaryValueLiteralOrList::List(items) => {
                    if matches!(monitor.value, Sb3MonitorValue::Primitive(..)) {
                        emit_message_eager(
                            ...
                            GrazeSourceMessage::Warning(
                                GrazeSourceWarning::Specific(
                                    SpecificGrazeWarning::ListValueForPrimitiveMonitor,
                    ...
                    monitor.value = Sb3MonitorValue::List(
                        items
                            .into_iter()
                            .map(|value| {
                                Ok(extract_data_from_dictionary_value!(
        ...
        if let Some(width) = extract_f64_entry_from_dictionary_data(context, &mut data, "width")? {
            monitor.width = width;
        }
        if let Some(height) = extract_f64_entry_from_dictionary_data(context, &mut data, "height")?
        {
        ...
        if let Some(slider_min) =
            extract_optional_f64_entry_from_dictionary_data(context, &mut data, "slider_min")?
        {
            monitor.slider_min = slider_min;
        }
        if let Some(slider_max) =
            extract_optional_f64_entry_from_dictionary_data(context, &mut data, "slider_max")?
        {
